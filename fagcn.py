...
class Net(torch.nn.Module):
    ...
    def forward(self, data, e_w, idx):
        ...
        x, x_init = F.dropout(F.relu(self.embed(x))), F.dropout(F.relu(self.embed(x)))
        
        x = self.fagcn(x, x_init, edge_index)
        x = self.fagcn(x, x_init, edge_index)
        x = self.fagcn(x, x_init, edge_index)
        x = self.fagcn(x, x_init, edge_index)
        x, eid = self.fagcn(x, x_init, edge_index, return_attention_weights=True)
        x_out = self.pred(x)
        ...
        return F.log_softmax(x_out, dim=1)#, x_out, err, f_e, eid
...
for ee in tqdm(range(epoch)):
    ...
    for eee in range(1000):    
        ...
        out = model(data, data.edge_index, 0)
        ...
        with torch.no_grad():
            model.eval()
            pred = model(data, data.edge_index, 0)
